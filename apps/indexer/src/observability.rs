...
pub fn init(service_name: &str) -> Result<OtelGuard> {
    ...
    // Default to a low sampling ratio (safe for production backfills).
    // Override with `OTEL_TRACES_SAMPLE_RATIO=1` for full tracing in dev.
    ...
    if otel_disabled {
        // No exporters; keep stdout logs working.
        ...
        tracing_subscriber::registry()
            .with(env_filter)
            .with(fmt_layer)
            // still install the layer so spans compile/emit; provider is AlwaysOff so no export
            .with(otel_layer)
            .init();
