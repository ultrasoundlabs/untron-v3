/*
Fix `pending_usdt_deposits` semantics in `api.lease_view`.

`api.receiver_usdt_transfer_actionability.recommended_action` semantics (see 0012):
- 'pre_entitle'            => a SUBJECTIVE_PRE_ENTITLE claim exists (origin=0); objective preEntitle still needed
- 'subjective_pre_entitle' => no claim yet and pre-entitle timing is still allowed

`api.lease_view.pending_usdt_deposits` is intended to show deposits that are NOT
yet reflected as hub claims, so it must filter on 'subjective_pre_entitle'.
*/
