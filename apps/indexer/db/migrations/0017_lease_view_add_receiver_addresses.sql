/*
Expose deterministic receiver addresses on `api.lease_view`.

The lease list endpoint (`/v3/lease_view`) is backed directly by `api.lease_view`.
It currently only exposes `receiver_salt`, but callers often need the actual
receiver address.

We already maintain `ctl.receiver_watchlist(receiver_salt -> receiver (Tron) / receiver_evm)`.
Join it into the view and surface:
- receiver_address_tron (T...)
- receiver_address_evm (0x...)

Note: this is a LEFT JOIN; if the worker hasn't populated the watchlist for a
salt yet, the address fields will be NULL.
*/
