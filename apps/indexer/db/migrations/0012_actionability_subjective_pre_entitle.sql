/*
Actionability: introduce `subjective_pre_entitle` stage.

New semantics for api.receiver_usdt_transfer_actionability.recommended_action:
- 'already_accounted'        => a PRE_ENTITLE claim exists for this tx_hash (objective proven)
- 'pre_entitle'              => a SUBJECTIVE_PRE_ENTITLE claim exists for this tx_hash (objective preEntitle still needed for reimbursement)
- 'subjective_pre_entitle'   => no claim yet and pre-entitle timing is allowed (operator should subjectively pre-entitle first)
- 'pull'                     => no claim yet but pre-entitle timing is no longer allowed (a later pull timestamp suggests preEntitle would revert)

Note: the relayer uses `recommended_action = 'pre_entitle'` to decide which deposits to prove.
*/
