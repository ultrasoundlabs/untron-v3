/*
Adds `pending_usdt_deposits` + summary fields to `api.lease_view`.

These are canonical TRC-20 USDT transfers into the lease's deterministic receiver
that:
- do not yet have a hub-side pre-entitle claim (origin in (0,1), origin_id == tx_hash), and
- are still time-eligible for `preEntitle` (transfer_ts > last_pull_ts), and
- best-effort attribute to this lease (expected_lease_id == lease_id).

This is intended to power realtor's `GET /leases/{lease_id}` aggregated response.
*/
