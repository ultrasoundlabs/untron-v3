...
${offset}${err}${errEnd}`;
    ...
    var Node3 = class _Node3 {
      static addStringTerminator(src, offset, str) {
        if (str[str.length - 1] === "\n") return str;
        const next = _Node3.endOfWhiteSpace(src, offset);
      ...
      static endOfBlockIndent(src, indent, lineStart) {
        const inEnd = _Node3.endOfIndent(src, lineStart);
        if (inEnd > lineStart + indent) {
          return inEnd;
        } else {
          const wsEnd = _Node3.endOfWhiteSpace(src, inEnd);
          const ch = src[wsEnd];
          if (!ch || ch === "\n") return wsEnd;
        }
        return null;
      ...
      static normalizeOffset(src, offset) {
        const ch = src[offset];
        return !ch ? offset : ch !== "\n" && src[offset - 1] === "\n" ? offset - 1 : _Node3.endOfWhiteSpace(src, offset);
      }
        ...
        while (ch === " " || ch === "	" || ch === "\n") {
          switch (ch) {
            ...
            case "	":
              ...
              offset = _Node3.endOfWhiteSpace(src, offset + 2) - 1;
              break;
            case " ":
              inCount += 1;
              offset += 1;
      ...
      commentHasRequiredWhitespace(start) {
        ...
        return start !== end || _Node3.atBlank(src, end - 1);
      }
      get hasComment() {
        if (this.context) {
          const {
      ...
      parseComment(start) {
        ...
        if (src[start] === Char.COMMENT) {
          const end = _Node3.endOfLine(src, start + 1);
      ...
      toString() {
        ...
        return _Node3.addStringTerminator(src, range.end, str);
      }
    };
    var YAMLError = class extends Error {
      constructor(name, source, message) {
        if (!message || !(source instanceof Node3)) throw new Error(`Invalid arguments for new ${name}`);
        super();
        this.name = name;
        this.message = message;
        this.source = source;
...
`;
    ...
    var PlainValue = class _PlainValue extends Node3 {
      ...
      parseBlockValue(start) {
        ...
        for (let ch = src[offset]; ch === "\n"; ch = src[offset]) {
          if (Node3.atDocumentBoundary(src, offset + 1)) break;
...
` + comment.replace(/^/gm, `${indent || ""}#`);
    ...
    var Scalar = class extends Node3 {
      constructor(value) {
        super();
        this.value = value;
      }
    ...
    var Collection = class _Collection extends Node3 {
      constructor(schema) {
        super();
...
${indent}${s}` : "\n";
    ...
    var stringifyKey = (key, jsKey, ctx) => {
      ...
      if (key instanceof Node3 && ctx && ctx.doc) return key.toString({
    ...
    var Pair = class _Pair extends Node3 {
      ...
      set commentBefore(cb) {
        ...
        if (this.key instanceof Node3) this.key.commentBefore = cb;
        else {
      ...
      toString(ctx, onComment, onChompKeep) {
        ...
        let explicitKey = !simpleKeys && (!key || keyComment || (key instanceof Node3 ? key instanceof Collection || key.type === PlainValue.Type.BLOCK_FOLDED || key.type === PlainValue.Type.BLOCK_LITERAL : typeof key === "object"));
        const {
          doc,
          indent,
          indentStep,
...
${ctx.indent}`;
    ...
    var Alias = class _Alias extends Node3 {
      static stringify({
        range,
        source
      }, {
...
async function fetchJson(url, opts) {
  ...
  const timeout = setTimeout(() => ctrl.abort(), opts?.timeoutMs ?? 1e4);
  try {
...
app.get("/openapi.json", async (c) => {
  const now = Date.now();
  const fresh = cache.body !== null && now - cache.atMs <= CACHE_SECONDS * 1e3;
  if (!fresh) {
