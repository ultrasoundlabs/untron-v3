...
/// struct for typed errors of method [`controller_receivers_get`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum ControllerReceiversGetError {
    UnknownValue(serde_json::Value),
}


pub async fn controller_receivers_get(configuration: &configuration::Configuration, receiver_salt: Option<&str>, valid_from_seq: Option<&str>, valid_to_seq: Option<&str>, receiver: Option<&str>, select: Option<&str>, order: Option<&str>, range: Option<&str>, range_unit: Option<&str>, offset: Option<&str>, limit: Option<&str>, prefer: Option<&str>) -> Result<Vec<models::ControllerReceivers>, Error<ControllerReceiversGetError>> {
    ...
    let uri_str = format!("{}/controller_receivers", configuration.base_path);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `Vec&lt;models::ControllerReceivers&gt;`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `Vec&lt;models::ControllerReceivers&gt;`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<ControllerReceiversGetError> = serde_json::from_str(&content).ok();
