...
/// struct for typed errors of method [`hub_tokens_rescued_get`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum HubTokensRescuedGetError {
    UnknownValue(serde_json::Value),
}


pub async fn hub_tokens_rescued_get(configuration: &configuration::Configuration, event_seq: Option<&str>, token: Option<&str>, amount: Option<&str>, select: Option<&str>, order: Option<&str>, range: Option<&str>, range_unit: Option<&str>, offset: Option<&str>, limit: Option<&str>, prefer: Option<&str>) -> Result<Vec<models::HubTokensRescued>, Error<HubTokensRescuedGetError>> {
    ...
    let uri_str = format!("{}/hub_tokens_rescued", configuration.base_path);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `Vec&lt;models::HubTokensRescued&gt;`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `Vec&lt;models::HubTokensRescued&gt;`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<HubTokensRescuedGetError> = serde_json::from_str(&content).ok();
