...
/// struct for typed errors of method [`controller_usdt_get`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum ControllerUsdtGetError {
    UnknownValue(serde_json::Value),
}


pub async fn controller_usdt_get(configuration: &configuration::Configuration, valid_from_seq: Option<&str>, valid_to_seq: Option<&str>, usdt: Option<&str>, select: Option<&str>, order: Option<&str>, range: Option<&str>, range_unit: Option<&str>, offset: Option<&str>, limit: Option<&str>, prefer: Option<&str>) -> Result<Vec<models::ControllerUsdt>, Error<ControllerUsdtGetError>> {
    ...
    let uri_str = format!("{}/controller_usdt", configuration.base_path);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `Vec&lt;models::ControllerUsdt&gt;`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `Vec&lt;models::ControllerUsdt&gt;`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<ControllerUsdtGetError> = serde_json::from_str(&content).ok();
