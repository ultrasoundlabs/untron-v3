...
/// struct for typed errors of method [`stream_cursor_get`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum StreamCursorGetError {
...
pub async fn stream_cursor_get(configuration: &configuration::Configuration, stream: Option<&str>, applied_through_seq: Option<&str>, tip: Option<&str>, updated_at: Option<&str>, select: Option<&str>, order: Option<&str>, range: Option<&str>, range_unit: Option<&str>, offset: Option<&str>, limit: Option<&str>, prefer: Option<&str>) -> Result<Vec<models::StreamCursor>, Error<StreamCursorGetError>> {
    ...
    let uri_str = format!("{}/stream_cursor", configuration.base_path);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `Vec&lt;models::StreamCursor&gt;`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `Vec&lt;models::StreamCursor&gt;`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<StreamCursorGetError> = serde_json::from_str(&content).ok();
