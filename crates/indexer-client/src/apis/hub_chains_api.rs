...
/// struct for typed errors of method [`hub_chains_get`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum HubChainsGetError {
    UnknownValue(serde_json::Value),
}


/// Deprecated destination chains cannot be selected in new payout configs.
pub async fn hub_chains_get(configuration: &configuration::Configuration, target_chain_id: Option<&str>, valid_from_seq: Option<&str>, valid_to_seq: Option<&str>, deprecated: Option<&str>, select: Option<&str>, order: Option<&str>, range: Option<&str>, range_unit: Option<&str>, offset: Option<&str>, limit: Option<&str>, prefer: Option<&str>) -> Result<Vec<models::HubChains>, Error<HubChainsGetError>> {
    ...
    let p_query_deprecated = deprecated;
    ...
    let uri_str = format!("{}/hub_chains", configuration.base_path);
    ...
    if let Some(ref param_value) = p_query_deprecated {
        req_builder = req_builder.query(&[("deprecated", &param_value.to_string())]);
    }
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `Vec&lt;models::HubChains&gt;`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `Vec&lt;models::HubChains&gt;`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<HubChainsGetError> = serde_json::from_str(&content).ok();
