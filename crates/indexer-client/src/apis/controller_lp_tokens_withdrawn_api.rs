...
/// struct for typed errors of method [`controller_lp_tokens_withdrawn_get`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum ControllerLpTokensWithdrawnGetError {
    UnknownValue(serde_json::Value),
}


pub async fn controller_lp_tokens_withdrawn_get(configuration: &configuration::Configuration, event_seq: Option<&str>, token: Option<&str>, amount: Option<&str>, select: Option<&str>, order: Option<&str>, range: Option<&str>, range_unit: Option<&str>, offset: Option<&str>, limit: Option<&str>, prefer: Option<&str>) -> Result<Vec<models::ControllerLpTokensWithdrawn>, Error<ControllerLpTokensWithdrawnGetError>> {
    ...
    let uri_str = format!("{}/controller_lp_tokens_withdrawn", configuration.base_path);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `Vec&lt;models::ControllerLpTokensWithdrawn&gt;`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `Vec&lt;models::ControllerLpTokensWithdrawn&gt;`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<ControllerLpTokensWithdrawnGetError> = serde_json::from_str(&content).ok();
