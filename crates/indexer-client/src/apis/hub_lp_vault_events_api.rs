...
/// struct for typed errors of method [`hub_lp_vault_events_get`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum HubLpVaultEventsGetError {
    UnknownValue(serde_json::Value),
}


pub async fn hub_lp_vault_events_get(configuration: &configuration::Configuration, event_seq: Option<&str>, kind: Option<&str>, lp: Option<&str>, amount: Option<&str>, select: Option<&str>, order: Option<&str>, range: Option<&str>, range_unit: Option<&str>, offset: Option<&str>, limit: Option<&str>, prefer: Option<&str>) -> Result<Vec<models::HubLpVaultEvents>, Error<HubLpVaultEventsGetError>> {
    ...
    let uri_str = format!("{}/hub_lp_vault_events", configuration.base_path);
    ...
    if let Some(ref param_value) = p_query_lp {
        req_builder = req_builder.query(&[("lp", &param_value.to_string())]);
    }
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `Vec&lt;models::HubLpVaultEvents&gt;`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `Vec&lt;models::HubLpVaultEvents&gt;`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<HubLpVaultEventsGetError> = serde_json::from_str(&content).ok();
