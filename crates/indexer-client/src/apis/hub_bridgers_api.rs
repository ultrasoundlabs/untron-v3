...
/// struct for typed errors of method [`hub_bridgers_get`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum HubBridgersGetError {
    UnknownValue(serde_json::Value),
}


/// If a claim targets a different chain than the hub chain, the configured bridger is used to deliver funds.
pub async fn hub_bridgers_get(configuration: &configuration::Configuration, target_token: Option<&str>, target_chain_id: Option<&str>, valid_from_seq: Option<&str>, valid_to_seq: Option<&str>, bridger: Option<&str>, select: Option<&str>, order: Option<&str>, range: Option<&str>, range_unit: Option<&str>, offset: Option<&str>, limit: Option<&str>, prefer: Option<&str>) -> Result<Vec<models::HubBridgers>, Error<HubBridgersGetError>> {
    ...
    let uri_str = format!("{}/hub_bridgers", configuration.base_path);
    ...
    if let Some(ref param_value) = p_query_bridger {
        req_builder = req_builder.query(&[("bridger", &param_value.to_string())]);
    }
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `Vec&lt;models::HubBridgers&gt;`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `Vec&lt;models::HubBridgers&gt;`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<HubBridgersGetError> = serde_json::from_str(&content).ok();
