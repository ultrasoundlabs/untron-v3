...
/// struct for typed errors of method [`hub_lp_allowlist_get`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum HubLpAllowlistGetError {
    UnknownValue(serde_json::Value),
}


/// LPs must be allowlisted to deposit principal into the hub fast-fill vault.
pub async fn hub_lp_allowlist_get(configuration: &configuration::Configuration, lp: Option<&str>, valid_from_seq: Option<&str>, valid_to_seq: Option<&str>, allowed: Option<&str>, select: Option<&str>, order: Option<&str>, range: Option<&str>, range_unit: Option<&str>, offset: Option<&str>, limit: Option<&str>, prefer: Option<&str>) -> Result<Vec<models::HubLpAllowlist>, Error<HubLpAllowlistGetError>> {
    ...
    let uri_str = format!("{}/hub_lp_allowlist", configuration.base_path);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `Vec&lt;models::HubLpAllowlist&gt;`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `Vec&lt;models::HubLpAllowlist&gt;`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<HubLpAllowlistGetError> = serde_json::from_str(&content).ok();
