...
/// struct for typed errors of method [`controller_payloads_get`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum ControllerPayloadsGetError {
    UnknownValue(serde_json::Value),
}


/// Payloads configure how USDT is bridged out of Tron for each rebalancer implementation.
pub async fn controller_payloads_get(configuration: &configuration::Configuration, rebalancer: Option<&str>, valid_from_seq: Option<&str>, valid_to_seq: Option<&str>, payload: Option<&str>, select: Option<&str>, order: Option<&str>, range: Option<&str>, range_unit: Option<&str>, offset: Option<&str>, limit: Option<&str>, prefer: Option<&str>) -> Result<Vec<models::ControllerPayloads>, Error<ControllerPayloadsGetError>> {
    ...
    let uri_str = format!("{}/controller_payloads", configuration.base_path);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `Vec&lt;models::ControllerPayloads&gt;`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `Vec&lt;models::ControllerPayloads&gt;`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<ControllerPayloadsGetError> = serde_json::from_str(&content).ok();
