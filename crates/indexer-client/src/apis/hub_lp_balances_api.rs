...
/// struct for typed errors of method [`hub_lp_balances_get`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum HubLpBalancesGetError {
    UnknownValue(serde_json::Value),
}


/// Derived deterministically from hub `LpDeposited`/`LpWithdrawn` events.
pub async fn hub_lp_balances_get(configuration: &configuration::Configuration, lp: Option<&str>, valid_from_seq: Option<&str>, valid_to_seq: Option<&str>, balance: Option<&str>, select: Option<&str>, order: Option<&str>, range: Option<&str>, range_unit: Option<&str>, offset: Option<&str>, limit: Option<&str>, prefer: Option<&str>) -> Result<Vec<models::HubLpBalances>, Error<HubLpBalancesGetError>> {
    ...
    let uri_str = format!("{}/hub_lp_balances", configuration.base_path);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `Vec&lt;models::HubLpBalances&gt;`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `Vec&lt;models::HubLpBalances&gt;`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<HubLpBalancesGetError> = serde_json::from_str(&content).ok();
