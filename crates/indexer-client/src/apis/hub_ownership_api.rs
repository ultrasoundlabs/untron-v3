...
/// struct for typed errors of method [`hub_ownership_get`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum HubOwnershipGetError {
    UnknownValue(serde_json::Value),
}


/// Derived from `OwnershipTransferred` events emitted via `UntronV3Index`.
pub async fn hub_ownership_get(configuration: &configuration::Configuration, valid_from_seq: Option<&str>, old_owner: Option<&str>, new_owner: Option<&str>, select: Option<&str>, order: Option<&str>, range: Option<&str>, range_unit: Option<&str>, offset: Option<&str>, limit: Option<&str>, prefer: Option<&str>) -> Result<Vec<models::HubOwnership>, Error<HubOwnershipGetError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_query_valid_from_seq = valid_from_seq;
    let p_query_old_owner = old_owner;
    let p_query_new_owner = new_owner;
    ...
    let uri_str = format!("{}/hub_ownership", configuration.base_path);
    ...
    if let Some(ref param_value) = p_query_old_owner {
        req_builder = req_builder.query(&[("old_owner", &param_value.to_string())]);
    }
    if let Some(ref param_value) = p_query_new_owner {
        req_builder = req_builder.query(&[("new_owner", &param_value.to_string())]);
    }
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `Vec&lt;models::HubOwnership&gt;`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `Vec&lt;models::HubOwnership&gt;`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<HubOwnershipGetError> = serde_json::from_str(&content).ok();
