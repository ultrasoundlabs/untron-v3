...
/// struct for typed errors of method [`controller_lp_exchange_rates_get`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum ControllerLpExchangeRatesGetError {
    UnknownValue(serde_json::Value),
}


/// Used by controller sweeps to compute USDT-equivalent amounts.
pub async fn controller_lp_exchange_rates_get(configuration: &configuration::Configuration, token: Option<&str>, valid_from_seq: Option<&str>, valid_to_seq: Option<&str>, exchange_rate: Option<&str>, select: Option<&str>, order: Option<&str>, range: Option<&str>, range_unit: Option<&str>, offset: Option<&str>, limit: Option<&str>, prefer: Option<&str>) -> Result<Vec<models::ControllerLpExchangeRates>, Error<ControllerLpExchangeRatesGetError>> {
    ...
    let uri_str = format!("{}/controller_lp_exchange_rates", configuration.base_path);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `Vec&lt;models::ControllerLpExchangeRates&gt;`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `Vec&lt;models::ControllerLpExchangeRates&gt;`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<ControllerLpExchangeRatesGetError> = serde_json::from_str(&content).ok();
