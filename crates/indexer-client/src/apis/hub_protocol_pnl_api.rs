...
/// struct for typed errors of method [`hub_protocol_pnl_get`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum HubProtocolPnlGetError {
    UnknownValue(serde_json::Value),
}


/// Tracks fee revenue and rebalance/withdrawal deltas as emitted by the hub contract.
pub async fn hub_protocol_pnl_get(configuration: &configuration::Configuration, valid_from_seq: Option<&str>, valid_to_seq: Option<&str>, pnl: Option<&str>, delta: Option<&str>, reason: Option<&str>, select: Option<&str>, order: Option<&str>, range: Option<&str>, range_unit: Option<&str>, offset: Option<&str>, limit: Option<&str>, prefer: Option<&str>) -> Result<Vec<models::HubProtocolPnl>, Error<HubProtocolPnlGetError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_query_valid_from_seq = valid_from_seq;
    let p_query_valid_to_seq = valid_to_seq;
    let p_query_pnl = pnl;
    let p_query_delta = delta;
    ...
    let uri_str = format!("{}/hub_protocol_pnl", configuration.base_path);
    ...
    if let Some(ref param_value) = p_query_pnl {
        req_builder = req_builder.query(&[("pnl", &param_value.to_string())]);
    }
    if let Some(ref param_value) = p_query_delta {
        req_builder = req_builder.query(&[("delta", &param_value.to_string())]);
    }
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `Vec&lt;models::HubProtocolPnl&gt;`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `Vec&lt;models::HubProtocolPnl&gt;`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<HubProtocolPnlGetError> = serde_json::from_str(&content).ok();
