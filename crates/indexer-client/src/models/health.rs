...
/// Health : Health check endpoint  Always returns a single row with `status = 'ok'` if PostgREST can query the database.
...
impl Health {
    /// Health check endpoint  Always returns a single row with `status = 'ok'` if PostgREST can query the database.
    pub fn new() -> Health {
        Health {
            status: None,
        }
