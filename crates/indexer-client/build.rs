...
fn sanitize_openapi_json(doc: &mut serde_json::Value) {
                ...
                if let Some(serde_json::Value::Array(types)) = map.get("type") {
                    ...
                    if has_null {
                        ...
                        if let Some(non_null) = non_null {
                            map.insert("type".to_string(), serde_json::Value::String(non_null.to_string()));
    ...
    if let Some(paths) = doc.get_mut("paths").and_then(serde_json::Value::as_object_mut) {
...
fn main() {
    ...
    let mut raw: serde_json::Value = serde_json::from_reader(file).expect("parse openapi.json as JSON");
    sanitize_openapi_json(&mut raw);

    let spec: openapiv3::OpenAPI = serde_path_to_error::deserialize(&mut serde_json::Deserializer::from_str(
        &raw.to_string(),
    ))
